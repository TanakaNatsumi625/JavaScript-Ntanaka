...
async function main() {
    ...
    if (command === "list") {
        ...
        const issues = await listIssues(owner, repoName);
        ...
        const issue = await createIssue(owner, repoName, title, body);
        ...
        const issue = await closeIssue(owner, repoName, issueNumber);
        console.log(`Closed issue #${issue.number}: ${issue.title}`);
    } else if (command === "me") {
        console.log("自分の情報を表示します");
        // ここでGitHub APIを呼び出して自分の情報を取得し、表示する処理を書く
        // これはテスト用
        const me = await gitHubAPIRequest("/user");
