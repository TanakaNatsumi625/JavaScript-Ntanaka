...
// sessionStorageが使えるか使えないかで呼び出す関数を分ける
if (!sessionStorageAvailable) {
  console.warn("sessionStorage is not available. ToDo list functionality will be limited.");
  //UIのみ変更する仕様の関数を呼び出す
  toDoListWithoutLocalStorage();
}else {
  //sessionStorageを使った仕様の関数を呼び出す
  toDoListWithSessionStorage();
}
...
function toDoListWithSessionStorage() {
  document.addEventListener("DOMContentLoaded", () => {
    if (!sessionStorageAvailable) {//もしセッションストレージが使えないなら
      return;//この関数はスキップ
    }
    // セッションストレージからToDoリストを読み込む
    const items = loadToDoListFromSessionStorage();
  ...
  form.addEventListener("submit", (e) => {
    ...
    const existingItems = loadToDoListFromSessionStorage();
  ...
  function appendToDoItem(item) {
    ...
    destroy.addEventListener("click", () => {
      const id = item.id;
      //セッションストレージからToDoリストを削除
      const existingItems = loadToDoListFromSessionStorage();//現在のセッションストレージの内容を取得
...
/**
 * sessionStorageからToDoリストを読み込む関数
 * @return {Array<string>} もしToDoリストがなければ空の配列を返す
 * @return {Array<string>} 読み込んだToDoリストのアイテム
 * 期待する型: { tasks: Array<string>, currentId: number }
 */
function loadToDoListFromSessionStorage() {
  const items = JSON.parse(sessionStorage.getItem("todoList"));
  if (!items) {
...
/**
 * SessionStorageを使って、ToDoのリストを保存する関数
 * @param  {Array<string>} items ToDoリストのアイテム
