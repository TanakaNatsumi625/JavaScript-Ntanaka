/**
* このクラスは、<input>テキスト入力フィールドと、2 つのアイコンまたは絵文字を
* 表示するカスタムHTML <search-box>要素を定義する。デフォルトでは、テキスト
* 入力フィールドの左側に虫眼鏡の絵文字（検索を表す）、右側にX の絵文字（キャンセル
* を示す）を表示する。テキスト入力フィールドのボーダーを隠し、2 つの絵文字が
...
*
* <search-box>は通常のHTML のdisabled とhidden 属性をサポートする。また、size 属性と
* placeholder 属性を利用できる。それぞれ、<input>要素と同じ意味になる。
ときに「clear」イベントが発生する。
* イベントハンドラがpreventDefault() を呼び出さなかった場合、ユーザの入力を
* クリアしてイベントのディスパッチが完了する。
*
* なお、onsearch やonclear というプロパティや属性はない。
...
// シャドウDOM ツリーを作成し、シャドウDOM をこの要素に接続する。
// this.shadowRoot の値に設定する。
this.attachShadow({mode: "open"});
// このカスタムコンポーネント用の子孫要素やスタイルシートを定義している
// テンプレートを複製し、コンテンツをシャドウルートに追加する。
this.shadowRoot.append(SearchBox.template.content.cloneNode(true));
// シャドウDOM 中の重要な要素の参照を取得する。
...
// イベントを発生させる（「change」イベントは、シャドウDOM の外側には
// バブリングしない。
leftSlot.onclick = this.input.onchange = (event) => {
    event.stopPropagation(); // click イベントがバブリングしないようにする。
if (this.disabled) return; // disabled のときは何もしない。
this.dispatchEvent(new CustomEvent("search", {
detail: this.input.value
}));
};
// ユーザがX をクリックした場合、「clear」イベントを発生させる。
// イベントに対してpreventDefault() が呼び出されなかった場合、入力をクリアする。
rightSlot.onclick = (event) => {
event.stopPropagation(); // click イベントがバブリングしないようにする。
if (this.disabled) return; // disabled の場合何もしない。
...
// 最後に、サポートするHTML 属性に対応したプロパティに対して、ゲッターとセッター
// を定義する。ゲッターは属性の値（または、属性の有無）を返すだけ。また、
// セッターは属性の値（または、属性の有無）を設定するだけ。セッターが属性を
// 変更するときに、ブラウザは自動的に前記したattributeChangedCallback を
// 自動的に呼び出す。
...
// この静的なフィールドは、attributeChangedCallback メソッドで必要になる。
...
// 保持する。SearchBox 要素のインスタンスで利用する。
SearchBox.template = document.createElement("template");
// このHTML 文字列を解釈してテンプレートを初期化する。しかし、SearchBox の
// インスタンスを作成するときには、テンプレートのノードを複製するだけ。
// HTML を再度解釈する必要はない。
SearchBox.template.innerHTML = `
<style>
/*
* :host セレクタはライトDOM での<search-box>要素を参照する。このスタイルは
* デフォルトの設定。<search-box>のユーザはライトDOM のスタイルでオーバライド
* できる。
*/
:host {
...
:host([hidden]) { /* 丸括弧に注意：ホストにhidden が設定されている場合*/
display:none; /* 属性を設定して表示しないようにする。*/
}
:host([disabled]) { /* ホストがdiabled 属性を持つ場合、*/
opacity: 0.5; /* グレー表示する。*/
}
:host([focused]) { /* ホストがfocused 属性を持つ場合、*/
box-shadow: 0 0 2px 2px #6AE; /* フォーカスリングを表示する。*/
}
/* 以下のスタイルシートは、シャドウDOM 中の要素にのみ適用される。*/
input {
...
<div>
<slot name="left">\u{1f50d}</slot> <!-- U+1F50D は虫眼鏡。-->
<input type="text" id="input" /> <!-- 実際の入力要素。-->
<slot name="right">\u{2573}</slot> <!-- U+2573 はX。-->
</div>
`;
// 最後に、customElement.define() を呼び出して、SearchBox 要素を<search-box>タグの
